            ...
            <aside class="col-span-1">
                <form action="{{ route('book.search') }}" method="GET" id="filterForm">
                    ...
                    <div class="bg-white p-6 rounded-lg border border-gray-200 shadow-md space-y-8">
                        <h4 class="text-xl font-semibold text-gray-800 border-b border-gray-200 pb-4">Filtrer les livres
                        </h4>
                                ...
                                @foreach ($allTags as $tag)
                                    <label class="flex items-center text-gray-600">
                                        <input type="checkbox" name="tags[]" value="{{ $tag }}"
                                            {{ in_array($tag, request('tags', [])) ? 'checked' : '' }}
                                            onchange="document.getElementById('filterForm').submit()"
                                            class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        ...
                        <div class="pt-4">
                            <a href="{{ route('book.search') }}"
                                class="w-full inline-block text-center px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">
